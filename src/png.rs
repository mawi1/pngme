...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        if let Some(index) = self.chunks.iter().position(|c| *c.chunk_type() == ct) {
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        ...
        self.chunks.iter().find(|&c| *c.chunk_type() == ct)
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        if bytes[0..Self::STANDARD_HEADER.len()] != Self::STANDARD_HEADER {
            bail!("invalid header");
        }
        let mut chunks = Vec::new();
        let mut next_chunk_start = Self::STANDARD_HEADER.len();
        while next_chunk_start < bytes.len() {
            let chunk = Chunk::try_from(&bytes[next_chunk_start..])?;
            next_chunk_start += chunk.total_lenght();
            chunks.push(chunk);
        }
